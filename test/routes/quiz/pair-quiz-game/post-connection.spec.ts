...
describe('PairQuizGame: POST pair-game-quiz/pairs/connection', () => {
  ...
  describe('negative: POST pair-game-quiz/pairs/connection', () => {
    ...
    it(`should not Connect current user to existing random pending pair 
    or create new pair which will be waiting second player if token is missing`, async () => {
    ...
    it(`should not Connect current user to existing random pending pair 
    or create new pair which will be waiting second player if token is incorrect`, async () => {
      const getGameResponse = await agent
        .post(`/pair-game-quiz/pairs/connection`)
    ...
    create new pair which will be waiting second player If current user is already participating in active pair`, async () => {
      ...
      const tryConnectByAlreadyActiveUser = await agent
        .post(`/pair-game-quiz/pairs/connection`)
        .set('Authorization', `Bearer ${userToken1}`)
        .expect(403);

      expectErrorWithPath(
        tryConnectByAlreadyActiveUser,
        403,
        `/pair-game-quiz/pairs/connection`,
      );
    });
  ...
  describe('positive: POST pair-game-quiz/pairs/connection', () => {
    ...
    it(`should Connect current user to existing random pending pair or
    create new pair which will be waiting second player If current user is already participating in active pair`, async () => {
      await agent
        .post(`/pair-game-quiz/pairs/connection`)
        .set('Authorization', `Bearer ${userToken1}`)
