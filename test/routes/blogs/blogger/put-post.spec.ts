...
describe('Blogs Blogger: PUT blogger/blogs/{blogId}/posts/{postId}', () => {
  ...
  describe('negative: PUT blogger/blogs/{blogId}/posts/{postId}', () => {
    it(`should clear db`, async () => {
      await agent.delete(testing_allData_uri);
    });

    it(`should not Update post if token is missing`, async () => {
    ...
    it(`should not Update post if token is incorrect`, async () => {
      const response = await agent
        .put(`/blogger/blogs/${blogId1}/posts/${postId1}`)
    ...
    it(`should not Update post If the inputModel has incorrect values`, async () => {
      const response = await agent
        .put(`/blogger/blogs/${blogId1}/posts/${postId1}`)
        .set('Authorization', `Bearer ${userToken1}`)
        .send({
  ...
  describe('positive: Blogger: PUT blogger/blogs/{blogId}/posts/{postId}', () => {
    it(`should clear db`, async () => {
      await waitForIt();
      await agent.delete(testing_allData_uri);
    }, 15000);
    ...
    it(`should Update post`, async () => {
      await agent
        .put(`/blogger/blogs/${blogId1}/posts/${postId1}`)
        .set('Authorization', `Bearer ${userToken1}`)
        .send(createPostInput3)
        .expect(204);

      const updatedPost = await agent
        .get(`/posts/${postId1}`)
        .set('Authorization', `Bearer ${userToken1}`)
        .expect(200);
