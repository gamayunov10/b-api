...
import { updatedDescription } from '../../../base/utils/constants/posts.constants';

describe('Blogs Blogger: PUT blogger/blogs/:blogId', () => {
  ...
  describe('negative: PUT blogger/blogs/:blogId', () => {
    it(`should clear db`, async () => {
      await agent.delete(testing_allData_uri);
    });

    it(`should not Update blog if token is missing`, async () => {
      const response = await agent
        .put(`/blogger/blogs/${blogId1}`)
        // .set('Authorization', `Bearer ${userToken1}`)
        .send({
    ...
    it(`should not Update blog If the inputModel has incorrect values`, async () => {
      const response = await agent
        .put(`/blogger/blogs/${blogId1}`)
        .set('Authorization', `Bearer ${userToken1}`)
        .send({
    ...
    it(`should not Update blog If user try to update blog that doesn't belong to current user`, async () => {
      await agent
        .put('/blogger/blogs/' + blogId1)
  ...
  describe('positive: Blogger: PUT blogger/blogs/:blogId', () => {
    it(`should clear db`, async () => {
      await waitForIt();
      await agent.delete(testing_allData_uri);
    }, 15000);
